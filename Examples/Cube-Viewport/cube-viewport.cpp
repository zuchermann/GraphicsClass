...
GLdouble modelViewMatrixLocal[16];
GLdouble modelViewMatrixWorld[16];
...
void homePosition( )
{
  ...
  glLoadIdentity( );
  glGetDoublev(GL_MODELVIEW_MATRIX,modelViewMatrixLocal);
  glGetDoublev(GL_MODELVIEW_MATRIX,modelViewMatrixWorld);
...
void operate()
{
  ...
  switch (operation)
    ...
    case TRANSLATION  :
		      glLoadIdentity();
		      glMultMatrixd(modelViewMatrixLocal);
		      translate(axis);
		      glGetDoublev(GL_MODELVIEW_MATRIX,modelViewMatrixLocal);

		      glLoadIdentity();
		      translate(axis);
		      glMultMatrixd(modelViewMatrixWorld);
		      glGetDoublev(GL_MODELVIEW_MATRIX,modelViewMatrixWorld);
...
// Callback for processing reshape events.
{
  ...
  glLoadIdentity( );
  gluPerspective(115.0, (float)w/(float)(2*h), 0.8, 100.0 );
}
...

{
  ...
  glutCreateWindow( "Transforming Cube in Local & World Coordinates" );
