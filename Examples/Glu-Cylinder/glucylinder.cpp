...
/*-------------------------------------------------*/
/*  glucylinder.cpp : Wire Frame Cylinder          */
/*-------------------------------------------------*/
...
#define BOTTOM_RADIUS_INIT  .5
#define TOP_RADIUS_INIT  .5

typedef enum { ROTATION, POSITION, PROJECTION, RADIUS,
               FIELD, SLICES, STACKS, HOME } operationType;
typedef enum { X, Y, Z } axisType;
typedef enum { BOTTOM, TOP } radiusType;
...
void homePosition( )
  ...
  /*--------------------------------------*/
  /* Rotate the cylinder to the home position */
...

{
  ...
  bottomRadius = BOTTOM_RADIUS_INIT;
  topRadius = TOP_RADIUS_INIT;
  ...
  switch (operation)
    ...
    case RADIUS   :
                      switch (radius)
                        {
                          case BOTTOM :
                                   if (direction == UP) bottomRadius += RADSTEP;
                                   else bottomRadius -= RADSTEP;
                                   break;
                          case TOP :
                                   if (direction == UP) topRadius += RADSTEP;
                                   else topRadius -= RADSTEP;
                                   break;
			}
                      break;
...

{
  glClear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT );
   ...
   gluCylinder(cylinder,bottomRadius,topRadius,0.5,slices,stacks);
...
// Callback for processing radius submenu.
{
  ...
  switch ( item )
  {
    case 1 : radius = BOTTOM; break;
    case 2 : radius = TOP; break;
  }
}
...
// Routine for creating menus.
{
  ...
  glutAddMenuEntry( "Bottom Radius",      1 );
  glutAddMenuEntry( "Top Radius",      2 );

  glutCreateMenu( mainMenu );
  glutAddSubMenu( "Rotate Cylinder ...",  rotationSubMenuCode );
  glutAddSubMenu( "Position Camera ...", positionSubMenuCode );
  glutAddSubMenu( "Modify Radii ...", radiusSubMenuCode );
